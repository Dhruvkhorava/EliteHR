$(document).ready(function() {
    var table = $('#user-table').DataTable({
        ...
        "pageLength": 10,
        "columnDefs": [
            { "orderable": false, "targets": [window.showRoleColumn ? 3 : 2] }
        ]
    });

    // Move Add New button to toolbar
    var addNewBtn = $('#add-new-btn');
    if (addNewBtn.length) {
        $("div.toolbar").append('<a href="' + addNewBtn.attr('href') + '" class="btn btn-primary ms-2">' + addNewBtn.text() + '</a>');
        addNewBtn.remove();
    }

    table.on('draw', function() {
        if (typeof $('.bs-tooltip').tooltip === 'function') {
            $('.bs-tooltip').tooltip();
        }
    });
});
