                ...
                <div class="widget-content">
                    <div class="w-header mb-4 d-flex justify-content-between align-items-center">
                        ...
                        <div class="task-action">
                            <span
                                class="badge badge-light-primary px-3 py-2 fs-6">{{ now('Asia/Kolkata')->format('l, d M Y') }}</span>
                        </div>
                    </div>
                            ...
                            <div class="p-2">
                                @if ($user->shift)
                                    <h3 class="mb-1 font-weight-bold text-info">
                                        {{ \Carbon\Carbon::parse($user->shift->start_time)->format('h:i A') }} -
                                        {{ \Carbon\Carbon::parse($user->shift->end_time)->format('h:i A') }}</h3>
                                    <p class="text-muted mb-0 fw-medium">Assigned Shift: <span
                                            class="text-dark">{{ $user->shift->name }}</span></p>
                                @else
                        ...
                        <div class="col-md-4 px-lg-5">
                            <div class="text-center p-2">
                                    ...
                                    <form action="{{ route('attendance.check-in') }}" method="POST">
                                        ...
                                        <button type="submit" class="btn btn-primary btn-lg w-100 shadow-none py-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                ...
                                                class="feather feather-log-in me-2">
                                                <path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"></path>
                                                <polyline points="10 17 15 12 10 7"></polyline>
                                                <line x1="15" y1="12" x2="3" y2="12"></line>
                                            </svg> Confirm Check-In
                                        </button>
                                    </form>
                                    ...
                                    <div class="d-flex gap-2">
                                        @if (!$attendance->is_on_break)
                                            <form action="{{ route('attendance.start-break') }}" method="POST"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-warning btn-lg w-100 shadow-none py-3 h-100">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                        class="feather feather-coffee me-2">
                                        ...
                                        @else
                                            <form action="{{ route('attendance.stop-break') }}" method="POST"
                                                class="flex-grow-1">
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-info btn-lg w-100 shadow-none py-3 h-100">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                        class="feather feather-play me-2">
                                        ...
                                        <form action="{{ route('attendance.check-out') }}" method="POST"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="btn btn-danger btn-lg w-100 shadow-none py-3 h-100">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @if ($attendance && !$attendance->check_out)
                                    <div class="mt-4 p-3 rounded d-flex justify-content-between align-items-center"
                                        style="background: rgba(0, 171, 255, 0.05); border: 1px dashed rgba(0, 171, 255, 0.2);">
                                        <div class="d-flex align-items-center text-start">
                                            <div class="pulse-indicator-blue me-3" id="tracking-pulse"></div>
                                            <div>
                                                <span class="small fw-bold text-dark d-block" id="tracking-status">Live
                                                    Tracking Sync Active</span>
                                                <span class="text-muted" style="font-size: 11px;"
                                                    id="last-sync-time">Waiting for first GPS pulse...</span>
                                            </div>
                                        </div>
                                        <div id="tracking-error-icon" class="d-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                viewBox="0 0 24 24" fill="none" stroke="#e7515a" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="mt-4 border-top pt-4">
                        <div class="row align-items-center">
                            <div class="col-md-6 d-flex align-items-center">
                                ...
                                <span
                                    class="badge {{ $attendance && $attendance->status == 'late' ? 'badge-light-danger' : 'badge-light-success' }} px-3">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 layout-spacing">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <ul class="nav nav-pills attendance-tab-nav" id="pills-tab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="pills-log-tab" data-bs-toggle="pill"
                                data-bs-target="#pills-log" type="button" role="tab">Attendance Log</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="pills-request-tab" data-bs-toggle="pill"
                                data-bs-target="#pills-request" type="button" role="tab">
                                Attendance Requests <span class="badge badge-danger rounded-circle ms-1"
                                    style="padding: 2px 6px; font-size: 10px;">{{ $pendingRequestsCount }}</span>
                            </button>
                        </li>
                    </ul>
                </div>
            ...
            <div class="tab-content" id="pills-tabContent">
                ...
                <div class="tab-pane fade show active" id="pills-log" role="tabpanel">
                    <div class="widget widget-card-four mb-0"
                        style="padding: 0; border-bottom-left-radius: 0; border-bottom-right-radius: 0;">
                        <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                            <h5 class="mb-0 fw-bold">
                                {{ request('month') ? \Carbon\Carbon::createFromDate(null, request('month'), 1)->format('F Y') : 'Last 30 Days' }}
                            </h5>
                            <div class="d-flex gap-2">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('attendance.index', ['days' => 30]) }}"
                                        class="btn btn-filter {{ !request('month') ? 'active' : '' }}">30 Days</a>
                                    @php
                                        $displayMonths = [
                                    ...
                                    @foreach ($displayMonths as $num => $name)
                                        <a href="{{ route('attendance.index', ['month' => $num]) }}"
                                            class="btn btn-filter {{ request('month') == $num ? 'active' : '' }}">{{ $name }}</a>
                                    @endforeach
                                </div>
                                {{-- <div class="btn-group" role="group">
                                ...
                                <table class="table attendance-table">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($period as $date)
                                            ...
                                            @if ($leave)
                                                <tr class="row-leave" data-bs-toggle="tooltip" data-bs-placement="top"
                                                    title="Leave: {{ $leave->leaveType->name }} (Approved)">
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <span class="me-2">{{ $date->format('M d, D') }}</span>
                                                            <span class="badge badge-leave small px-1 py-0"
                                                                style="font-size: 10px;">LEAVE</span>
                                                        </div>
                                                    </td>
                                                    <td colspan="5" class="text-center py-3">
                                                        <span class="text-muted small fw-bold uppercase">ON
                                                            {{ strtoupper($leave->leaveType->name) }}</span>
                                                    </td>
                                                </tr>
                                            @elseif($isWeekend && !$record)
                                                <tr class="row-special">
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <span class="me-2">{{ $date->format('M d, D') }}</span>
                                                            <span class="badge badge-woff small px-1 py-0"
                                                                style="font-size: 10px;">W-OFF</span>
                                                        </div>
                                                    </td>
                                            ...
                                            @elseif(!$record && !$date->isToday())
                                                <tr>
                                                    ...
                                                    <td colspan="4" class="text-center py-3 text-muted small fw-bold">
                                                        NO TIME ENTRIES LOGGED</td>
                                            ...
                                            @else
                                                <tr
                                                    @if ($record) data-bs-toggle="tooltip" data-bs-placement="top" title="In: {{ $record->check_in ? \Carbon\Carbon::parse($record->check_in)->format('h:i A') : '-' }} | Out: {{ $record->check_out ? \Carbon\Carbon::parse($record->check_out)->format('h:i A') : 'Active' }}" @endif>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <span class="me-2">{{ $date->format('M d, D') }}</span>
                                                            @if ($record && $record->status == 'present' && $record->working_hours > 8)
                                                                <span class="badge badge-wfh small px-1 py-0"
                                                                    style="font-size: 10px;">WFH</span>
                                                            @endif
                                                        </div>
                                                    </td>
                                                    <td>
                                                        @if ($record)
                                                            @php
                                                                ...
                                                                $workWidth =
                                                                    ($workingSeconds / $totalShiftSeconds) * 100;
                                                                $breakWidth =
                                                                    ($breakSeconds / $totalShiftSeconds) * 100;
                                                                $startOffset = 25; // Simulating start at 9:00 or similar
                                                            @endphp
                                                            <div class="attendance-visual" data-bs-toggle="tooltip"
                                                                title="Worked: {{ number_format($record->working_hours, 2) }}h | Break: {{ round($breakSeconds / 60) }}m">
                                                                <div class="visual-segment segment-work"
                                                                    style="width: {{ $workWidth / 2 }}%; margin-left: {{ $startOffset }}%;">
                                                                </div>
                                                                @if ($breakWidth > 2)
                                                                    <div class="visual-segment segment-break"
                                                                        style="width: {{ $breakWidth }}%;"></div>
                                                                @endif
                                                                <div class="visual-segment segment-work"
                                                                    style="width: {{ $workWidth / 2 }}%;"></div>
                                                            </div>
                                                        ...
                                                        @else
                                                            <div class="text-center">
                                                                <span
                                                                    class="badge badge-light-primary rounded-pill px-3 py-1"
                                                                    style="font-size: 10px; cursor: pointer;">Apply
                                                                    Leave</span>
                                                            </div>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <div
                                                                class="hours-circle {{ $record && $record->working_hours >= 8 ? 'circle-full' : 'circle-half' }}">
                                                            </div>
                                                            <span>{{ $record ? number_format($record->working_hours, 2) : '0.00' }}
                                                                hrs</span>
                                                        </div>
                                                    </td>
                                                    <td>{{ $record ? number_format($record->working_hours, 2) : '0.00' }}
                                                        hrs</td>
                                                    <td>
                                                        @if ($record && $record->check_in)
                                                            @php
                                                                ...
                                                                if ($user->shift) {
                                                                    ...
                                                                    $shiftStartTime = \Carbon\Carbon::parse(
                                                                        $user->shift->start_time,
                                                                    )->addMinutes($user->shift->grace_period);
                                                            ...
                                                            @endphp
                                                            @if ($isLate)
                                                                <span class="text-dark small fw-bold">{{ $lateDiff }}
                                                                    Late <span style="font-size: 16px;">🐌</span></span>
                                                            @else
                                                    ...
                                                    <td class="text-center">
                                                        @if ($record && $record->check_out)
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="20"
                                                                ...
                                                                stroke="#00abff" stroke-width="2" stroke-linecap="round"
                                                        ...
                                                        @elseif($record)
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="20"
                                                                height="20" viewBox="0 0 24 24" fill="none"
                                                                stroke="#e2a03f" stroke-width="2" stroke-linecap="round"
                                    ...
                                    <table class="table attendance-table">
                                        ...
                                        <tbody>
                                            @foreach ($pendingRequests as $req)
                                                <tr>
                                                    <td><span
                                                            class="badge badge-leave rounded-pill px-3">{{ $req->leaveType->name }}</span>
                                                    </td>
                                                    <td>{{ \Carbon\Carbon::parse($req->from_date)->format('d M Y') }}</td>
                                                    <td>{{ \Carbon\Carbon::parse($req->to_date)->format('d M Y') }}</td>
                                                    <td>{{ $req->total_days }}</td>
                                                    <td><small
                                                            class="text-muted">{{ Str::limit($req->reason, 30) }}</small>
                                                    </td>
                        ...
                        <div class="widget widget-card-four text-center py-5">
                            <div class="py-5">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"
                                    viewBox="0 0 24 24" fill="none" stroke="#bfc9d4" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round">
                                ...
                                <p class="text-muted small">All your attendance adjustment and leave requests will appear
                                    here.</p>
                            </div>
                        </div>
                    @endif
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            @if ($attendance && !$attendance->check_out && !$attendance->is_on_break)
                window.initEliteHRTracker({
                    updateUrl: "{{ route('attendance.record-location') }}",
                    csrfToken: "{{ csrf_token() }}",
                    interval: 60000, // 1 minute for "live" feel
                    isCheckedIn: true
                });
            @endif
        });
